<?php

declare(strict_types=1);

namespace ChickMate;

final class Country
{
    public const GREAT_BRITAIN = 'great_britain';
    public const IRELAND = 'ireland';
    public const USA = 'usa';
    public const ITALY = 'italy';
    public const MALTA = 'malta';
    public const PORTUGAL = 'portugal';
    public const SPAIN = 'spain';
    public const FRANCE = 'france';
    public const BELGIUM = 'belgium';
    public const LUXEMBOURG = 'luxembourg';
    public const THE_NETHERLANDS = 'the_netherlands';
    public const EAST_FRISIA = 'east_frisia';
    public const GERMANY = 'germany';
    public const AUSTRIA = 'austria';
    public const SWISS = 'swiss';
    public const ICELAND = 'iceland';
    public const DENMARK = 'denmark';
    public const NORWAY = 'norway';
    public const SWEDEN = 'sweden';
    public const FINLAND = 'finland';
    public const ESTONIA = 'estonia';
    public const LATVIA = 'latvia';
    public const LITHUANIA = 'lithuania';
    public const POLAND = 'poland';
    public const CZECH_REPUBLIC = 'czech_republic';
    public const SLOVAKIA = 'slovakia';
    public const HUNGARY = 'hungary';
    public const ROMANIA = 'romania';
    public const BULGARIA = 'bulgaria';
    public const BOSNIA_AND_HERZEGOVINA = 'bosnia_and_herzegovina';
    public const CROATIA = 'croatia';
    public const KOSOVO = 'kosovo';
    public const MACEDONIA = 'macedonia';
    public const NORTH_MACEDONIA = 'north_macedonia';
    public const MONTENEGRO = 'montenegro';
    public const SERBIA = 'serbia';
    public const SLOVENIA = 'slovenia';
    public const ALBANIA = 'albania';
    public const GREECE = 'greece';
    public const RUSSIA = 'russia';
    public const BELARUS = 'belarus';
    public const MOLDOVA = 'moldova';
    public const UKRAINE = 'ukraine';
    public const ARMENIA = 'armenia';
    public const AZERBAIJAN = 'azerbaijan';
    public const GEORGIA = 'georgia';
    public const KAZAKHSTAN = 'kazakhstan';
    public const KYRGYZSTAN = 'kyrgyzstan';
    public const TAJIKISTAN = 'tajikistan';
    public const TURKMENISTAN = 'turkmenistan';
    public const UZBEKISTAN = 'uzbekistan';
    public const TURKEY = 'turkey';
    public const ARABIA = 'arabia';
    public const PERSIA = 'persia';
    public const IRAN = 'iran';
    public const ISRAEL = 'israel';
    public const CHINA = 'china';
    public const INDIA = 'india';
    public const SRI_LANKA = 'sri_lanka';
    public const JAPAN = 'japan';
    public const KOREA = 'korea';
    public const VIETNAM = 'vietnam';
    public const OTHER_COUNTRIES = 'other_countries';

    public const LIST = [
        self::GREAT_BRITAIN,
        self::IRELAND,
        self::USA,
        self::ITALY,
        self::MALTA,
        self::PORTUGAL,
        self::SPAIN,
        self::FRANCE,
        self::BELGIUM,
        self::LUXEMBOURG,
        self::THE_NETHERLANDS,
        self::GERMANY,
        self::EAST_FRISIA,
        self::AUSTRIA,
        self::SWISS,
        self::ICELAND,
        self::DENMARK,
        self::NORWAY,
        self::SWEDEN,
        self::FINLAND,
        self::ESTONIA,
        self::LATVIA,
        self::LITHUANIA,
        self::POLAND,
        self::CZECH_REPUBLIC,
        self::SLOVAKIA,
        self::HUNGARY,
        self::ROMANIA,
        self::BOSNIA_AND_HERZEGOVINA,
        self::CROATIA,
        self::KOSOVO,
        self::MACEDONIA,
        self::NORTH_MACEDONIA,
        self::MONTENEGRO,
        self::SERBIA,
        self::SLOVENIA,
        self::ALBANIA,
        self::GREECE,
        self::RUSSIA,
        self::BELARUS,
        self::MOLDOVA,
        self::UKRAINE,
        self::ARMENIA,
        self::AZERBAIJAN,
        self::GEORGIA,
        self::KAZAKHSTAN,
        self::KYRGYZSTAN,
        self::TAJIKISTAN,
        self::TURKMENISTAN,
        self::UZBEKISTAN,
        self::TURKEY,
        self::ARABIA,
        self::PERSIA,
        self::IRAN,
        self::ISRAEL,
        self::CHINA,
        self::INDIA,
        self::SRI_LANKA,
        self::JAPAN,
        self::KOREA,
        self::VIETNAM,
        self::OTHER_COUNTRIES
    ];
}
